...
:release
@SET user_elf=Release//%PROGRAM_NAME%.elf
@SET ubootspl=%BM_SRC_PATH%//u-boot-spl
@GOTO find_elf_entry_point

:debug
@SET user_elf=Debug//%PROGRAM_NAME%.elf
@SET ubootspl=%BM_SRC_PATH%//u-boot-spl-nocache
@GOTO find_elf_entry_point

:find_elf_entry_point
@FOR /F "tokens=4" %%a IN ('arm-none-eabi-readelf -h %user_elf% ^| FINDSTR "Entry point"') DO @SET user_entry=%%a
